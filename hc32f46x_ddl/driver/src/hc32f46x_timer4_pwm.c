...
{
    __IO stc_tmr4_pocru_field_t *pstcPOCR_f = NULL;
    ...
    if ((IS_VALID_TIMER4(TMR4x)) && (NULL != pstcInitCfg))
    {
        ...
        {
            case Timer4PwmU:
                pstcRCSR_f->RTIDU = (Enable == pstcInitCfg->enRtIntMaskCmd) ? (uint16_t)1:(uint16_t)0;
                break;
            case Timer4PwmV:
                pstcRCSR_f->RTIDV = (Enable == pstcInitCfg->enRtIntMaskCmd) ? (uint16_t)1:(uint16_t)0;
                break;
            case Timer4PwmW:
                pstcRCSR_f->RTIDW = (Enable == pstcInitCfg->enRtIntMaskCmd) ? (uint16_t)1:(uint16_t)0;
                break;
            default:
                enRet = ErrorInvalidParameter;
                break;
