                                ...
                                const stc_timer4_cnt_init_t *pstcInitCfg)
{
    ...
    if ((IS_VALID_TIMER4(TMR4x)) && (NULL != pstcInitCfg))
    {
        ...
        /* Set default value */
        TMR4x->CCSR = (uint16_t)0x0050;
        TMR4x->CNTR = (uint16_t)0x0000;
        TMR4x->CPSR = (uint16_t)0xFFFF;
        TMR4x->CVPR = (uint16_t)0x0000;
        ...
        CCSR_f.BUFEN = (Enable == pstcInitCfg->enBufferCmd) ? ((uint16_t)1) : ((uint16_t)0);

        /* set external clock enable bit */
        CCSR_f.ECKEN = (Timer4CntExtclk == pstcInitCfg->enClk) ? ((uint16_t)1) : ((uint16_t)0);

        /* Set interrupt enable */
        CCSR_f.IRQZEN = (Enable == pstcInitCfg->enZeroIntCmd) ? ((uint16_t)1) : ((uint16_t)0);
        CCSR_f.IRQPEN = (Enable == pstcInitCfg->enPeakIntCmd) ? ((uint16_t)1) : ((uint16_t)0);
                                ...
                                en_timer4_cnt_clk_t enCntClk)
{
    ...
    if (IS_VALID_TIMER4(TMR4x))
    {
        ...
        /* set external clock enable bit */
        TMR4x->CCSR_f.ECKEN = (Timer4CntExtclk == enCntClk) ? ((uint16_t)1) : ((uint16_t)0);
        enRet = Ok;
    }
