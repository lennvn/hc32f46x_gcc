...
#define TMR4_OCMRx(__TMR4x__, __CH__)       ((uint32_t)&(__TMR4x__)->OCMRUH + ((uint32_t)(__CH__))*4ul)
...
{
    __IO stc_tmr4_ocsru_field_t* pstcOCSR = NULL;
    __IO stc_tmr4_oceru_field_t* pstcOCER = NULL;
    ...
    if ((IS_VALID_TIMER4(TMR4x)) && (NULL != pstcInitCfg))
    {
        ...
        if (IS_VALID_OCO_HIGH_CH(enCh)) /* channel: Timer4OcoOuh, Timer4OcoOvh, Timer4OcoOwh */
        {
            ...
            if (enRet == Ok)
            {
                ...
                pstcOCSR->OCPHU = ((pstcInitCfg->enPortLevel == OcPortLevelHigh) ? (uint16_t)1 : (uint16_t)0);
                /* set interrupt enable */
                pstcOCSR->OCIEHU = (Enable == pstcInitCfg->enOcoIntCmd) ? (uint16_t)1 : (uint16_t)0;
            }
        }
        else if (IS_VALID_OCO_LOW_CH(enCh)) /* channel: Timer4OcoOul, Timer4OcoOvl, Timer4OcoOwl */
        {
            ...
            {
                /* Set initial OP level */
                pstcOCSR->OCPLU = (OcPortLevelHigh == pstcInitCfg->enPortLevel) ? (uint16_t)1 : (uint16_t)0;
                /* set interrupt enable */
                pstcOCSR->OCIELU = (Enable == pstcInitCfg->enOcoIntCmd) ? (uint16_t)1 : (uint16_t)0;
            }
        }
        else
        {
                                ...
                                en_functional_state_t enCmd)
{
    ...
    if (IS_VALID_TIMER4(TMR4x))
    {
        ...
        /* Get pointer of current channel OCO register address  */
        pstcOCER = (__IO stc_tmr4_oceru_field_t*)TMR4_OCERx(TMR4x, enCh);
        IS_VALID_OCO_HIGH_CH(enCh) ? (pstcOCER->MCECHU = (uint16_t)enCmd) : (pstcOCER->MCECLU = (uint16_t)enCmd);
                                ...
                                const stc_oco_high_ch_compare_mode_t *pstcMode)
{
    ...
    if ((IS_VALID_TIMER4(TMR4x)) && (NULL != pstcMode))
    {
        ...
        pstcOCER->MCECHU = (pstcMode->enMatchConditionExtendCmd == Enable) ? (uint16_t)1 : (uint16_t)0;
                                 ...
                                 const stc_oco_low_ch_compare_mode_t *pstcMode)
{
    ...
    if ((IS_VALID_TIMER4(TMR4x)) && (NULL != pstcMode))
    {
        ...
        pstcOCER->MCECLU = (pstcMode->enMatchConditionExtendCmd == Enable) ? (uint16_t)1 : (uint16_t)0;
                                ...
                                en_functional_state_t enCmd)
{
    ...
    if (IS_VALID_TIMER4(TMR4x))
    {
        ...
        /* set register */
        IS_VALID_OCO_HIGH_CH(enCh) ? (pstcOCSR->OCEHU = (uint16_t)enCmd) : (pstcOCSR->OCELU = (uint16_t)enCmd);
        enRet = Ok;
    }
                                ...
                                en_functional_state_t enCmd)
{
    ...
    if (IS_VALID_TIMER4(TMR4x))
    {
        ...
        pstcOCSR = (__IO stc_tmr4_ocsru_field_t*)TMR4_OCSRx(TMR4x, enCh);
        /* set register */
        IS_VALID_OCO_HIGH_CH(enCh) ? (pstcOCSR->OCIEHU = (uint16_t)enCmd) : (pstcOCSR->OCIELU = (uint16_t)enCmd);
                                ...
                                en_timer4_oco_port_level_t enLevel)
{
    ...
    if (IS_VALID_TIMER4(TMR4x))
    {
        ...
        u16Level = (OcPortLevelHigh == enLevel) ? (uint16_t)1 : (uint16_t)0;
        /* Get pointer of current channel OCO register address */
        pstcOCSR = (__IO stc_tmr4_ocsru_field_t*)TMR4_OCSRx(TMR4x, enCh);
        IS_VALID_OCO_HIGH_CH(enCh) ? (pstcOCSR->OCFHU = u16Level) : (pstcOCSR->OCFLU = u16Level);
