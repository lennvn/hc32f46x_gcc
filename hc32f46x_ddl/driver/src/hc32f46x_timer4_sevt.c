                                ...
                                const stc_timer4_sevt_init_t *pstcInitCfg)
{
    ...
    __IO stc_tmr4_scsruh_field_t *pstcSCSR_f = NULL;
    __IO stc_tmr4_scmruh_field_t *pstcSCMR_f = NULL;
    ...
    if ((IS_VALID_TIMER4(TMR4x)) && (NULL != pstcInitCfg))
    {
        ...
        *(__IO uint16_t*)pstcSCMR_f = 0xFF00u;
        ...
        if (Ok == enRet)
        {
            ...
            stcSCMR_f.MZCEUH = ((Enable == pstcInitCfg->enCmpAmcZicCmd) ? (uint16_t)1 :(uint16_t)0);
            stcSCMR_f.MPCEUH = ((Enable == pstcInitCfg->enCmpAmcPicCmd) ? (uint16_t)1 :(uint16_t)0);
                                ...
                                const stc_timer4_sevt_trigger_cond_t *pstcTrigCond)
{
    ...
    if (IS_VALID_TIMER4(TMR4x))
    {
        ...
        /* Get actual address of register list of current channel */
        pstcSCSR_f = (__IO stc_tmr4_scsruh_field_t*)TMR4_SCSRx(TMR4x, enCh);
        pstcSCSR_f->PENUH = (Enable == pstcTrigCond->enPeakMatchCmd) ? (uint16_t)1 :(uint16_t)0;
        pstcSCSR_f->ZENUH = (Enable == pstcTrigCond->enZeroMatchCmd) ? (uint16_t)1 :(uint16_t)0;
        pstcSCSR_f->UENUH = (Enable == pstcTrigCond->enUpMatchCmd)   ? (uint16_t)1 :(uint16_t)0;
        pstcSCSR_f->DENUH = (Enable == pstcTrigCond->enDownMatchCmd) ? (uint16_t)1 :(uint16_t)0;
